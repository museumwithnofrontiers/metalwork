...
describe('website smoke test', () => {
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain("Metalwork")
  ...
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/ed60638d-d98c-52c9-b631-d363eb97fb51')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/e76dd0c9-2da5-504f-bb1b-e1aa38d9c4b4')
    ...
    const isl = await mountSite('#/item/ed60638d-d98c-52c9-b631-d363eb97fb51')
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === 'ed60638d-d98c-52c9-b631-d363eb97fb51')
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    const { app, host } = await mountSite('#/item/9759fb2e-169a-5a39-8d80-11289577854b')
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain('Qatar Museums')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/38b8a503-bba9-5234-b7db-0bbb2a054c57')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('Qatar Museums'), { timeout: 20000 })
    expect(host.textContent).toContain("Doha")
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/38b8a503-bba9-5234-b7db-0bbb2a054c57/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['metalwork', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
