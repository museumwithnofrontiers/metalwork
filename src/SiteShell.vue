...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('metalwork.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('metalwork.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('metalwork.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
